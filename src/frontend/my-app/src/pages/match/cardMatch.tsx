...
export const MatchesCard = ({ anketsData, prevCount, backCount, count, match }: { anketsData: any, prevCount: () => void, backCount: () => void, count: number, match: string[] }) => {
    ...
    const swipeHandlers = useSwipeable({
        onSwipedLeft: () => {
            if (count + 1 >= match.length) {
                setTextError('Вы можете cделать только свайп вправо.')
                setIsOpen(true)
                return
            }
            handleSwipe("left")
    ...
    return (
        <>
            <motion.div
                ...
                className={`mt-[30px] ${bgClasses[type]} ${shadowClasses[type]} flex flex-col justify-center items-center rounded-2xl shadow-[0px_0px_40px_0px_rgba(152,120,170,0.35)] transition-transform duration-300 ease-in-out`}
            >
                ...
                <div className="flex flex-col justify-center items-center">
...

                    {
                        message !== '' &&
                        <>
                            <div className={`text-[14px] ${textClasses[type]} mb-[10px] text-center break-all whitespace-pre-wrap w-full px-3`}>
                    ...
                    <div className="mb-[20px]">
                        ...
                        <div className={`${borderClasses[type]} ${titleClasses[type]} text-[16px] font-montserrat font-bold cursor-pointer transition-all duration-500 ease-out hover:border-[#5170DE] hover:text-[#5170DE]`}>
                            Написать
                        </div>
